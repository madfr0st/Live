...
class Treequery {
    ...
    public static void main(String[] args)throws IOException{
        ...
        int x = (int) (2e5);
        int[] given = new int[x];
        SegmentTree.MaxOperation operation = new SegmentTree.MaxOperation();
        ...
        for(int t=0;t<q;t++ ){
            ...
            if(a[3]==1){
                delta = 1;
                segmentTree.updateSegmentTreeRangeLazy(given,tree,lazy,qlow,qhigh,1,operation);
                segmentTree.updateSegmentTreeRangeLazy(given,tree,lazy,at,at,a[1],operation);
            }
            else{
                delta = a[1]+1;
                int atmax = segmentTree.rangeQueryLazy(tree,lazy,at,at,given.length,operation);
                int rangeMax = Math.max(segmentTree.rangeQueryLazy(tree,lazy,qlow,at-1,given.length,operation),
                        segmentTree.rangeQueryLazy(tree,lazy,at+1,qhigh,given.length,operation));
                //System.out.println(atmax + " "+rangeMax);

                delta = Math.max(atmax+delta,rangeMax+1)-Math.max(atmax,rangeMax);
                //System.out.println(delta);
                segmentTree.updateSegmentTreeRangeLazy(given,tree,lazy,qlow,qhigh,delta,operation);
            }
//            print(tree);
//            print(lazy);
//            System.out.println();
        }
        System.out.println(segmentTree.rangeQueryLazy(tree,lazy,1,given.length,given.length,operation));
    }
    static void print(int[] bin){
        for(int i=0;i<bin.length;i++){
            System.out.print(i+": "+bin[i]+", ");
        }
        System.out.println();
    }
}